...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    ArrayList<Long> rfs = new ArrayList<>();
    boolean validhw = false;
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            Long rf = Long.parseLong(attributes.getValue("ref"));
            rfs.add(rf);
            /*
        ...
        if (qName.equals("way")) {
            ...
            if (!validhw) {
                ...
                for (int i = 0; i < rfs.size() - 1; i++) {
                    Long fromvert = rfs.get(i);
                    Long tovert = rfs.get(i + 1);
                    double dist = g.distance(rfs.get(i), rfs.get(i + 1));
                    g.addedge(g.vertexids.get(fromvert), g.vertexids.get(tovert), dist);
                    g.addedge(g.vertexids.get(tovert), g.vertexids.get(fromvert), dist);

                }
                /* Hint: If you have stored the possible connections for this way,
